...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      console.log(`응답 상태: ${myResponse.status} ${myResponse.statusText}`);
      ...
      console.log(`요청 URL: https://open.api.nexon.com/fconline/v1/matches?game_type=40&offset=0&limit=100&nickname=${encodeURIComponent(myNickname)}`);
      
      const matchResponse = await fetch(`https://open.api.nexon.com/fconline/v1/matches?game_type=40&offset=0&limit=100&nickname=${encodeURIComponent(myNickname)}`, {
        headers: { 'Authorization': process.env.API_KEY }
      });
